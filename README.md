...
项目描述：一个面向校园的智能客服平台，旨在通过AI自动化处理大量用户咨询。系统采用Java微服务与Python AI服务混合架构。
...
注意：如需大模型生成回答，请修改 main.py 中的 DEEPSEEK_API_KEY 为你的 Key。不设置 Key 时大模型接口调用会失败，AI 服务将返回“AI 服务暂时不可用”的提示。
...
mvn spring-boot:run
4. 打开前端
建议使用 VS Code Live Server 打开 frontend/index.html（端口 5500），网关已配置对应 CORS。也可直接双击打开，但可能遇到跨域限制。

调用流程
前端生成 sessionId，建立 WebSocket 连接 ws://localhost:9001/ws/qa/{sessionId}；

用户提问后，前端 POST http://localhost:8080/api/qa/ask，携带 { sessionId, question }；

网关将请求路由到 campus-service，问题 JSON 写入 Redis 队列 campus:qa:queue，接口立即返回“已受理”；

消费线程从队列取出问题，读取最近 5 条会话历史，通过 Feign 调用 Python AI 服务 /ask；

AI 服务对 faq.txt 做向量检索，将命中的知识 + 历史拼进 Prompt 调 DeepSeek 生成回答；

回答写入会话历史，并通过 WebSocket 推送给前端。
...
WS	/ws/qa/{sessionId}	WebSocket 长连接，服务端推送 { "type": "answer", "question": "...", "answer": "..." }
...
常见问题
AI 回答提示“服务不可用”：多为 DeepSeek Key 未配置或调用失败，请检查 Key 是否有效；知识库未加载时请确认 faq.txt 与 main.py 在同一目录。
